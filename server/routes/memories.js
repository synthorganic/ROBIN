 ...
 */
function deleteSectionFromLines(lines, sectionTitle) {
    let sectionStart = -1;
 ...
 */
function deleteItemFromLines(lines, itemText) {
...
function getMemoryFilePath(workspace, date) {
    if (date) {
        return path.join(workspace.memoryDir, `${date}.md`);
    }
    return workspace.memoryPath;
}
function getMutexKey(agentId) {
    return `memory-file:${agentId}`;
}
function resolveWorkspaceOrResponse(c, agentId) {
    try {
 ...
 */
async function deleteMemory(opts, workspace) {
    ...
    try {
        ...
        console.error('[memories] Failed to delete:', err.message);
        return { deleted: false };
    }
}
 ...
 */
async function appendToMemoryFile(text, section, filePath) {
    let content = '';
    try {
    ...
    if (!content) {
        ...
        if (!isLocal) {
            try {
                ...
                console.warn('[memories] Gateway MEMORY.md fallback failed:', err.message);
            }
        }
    }
    if (content) {
...
app.get('/api/memories/section', rateLimitGeneral, async (c) => {
    ...
    try {
        ...
        console.error('[memories] GET section error:', err.message);
 ...
 */
app.post('/api/memories', rateLimitGeneral, zValidator('json', createMemorySchema, (result, c) => {
    if (!result.success) {
    ...
    try {
        ...
        if (isLocal) {
            ...
            try {
                ...
                console.error('[memories] Gateway MEMORY.md write failed:', err.message);
        ...
        try {
            ...
            console.warn('[memories] Gateway memory_store failed (non-fatal):', err.message);
        }
...
app.put('/api/memories/section', rateLimitGeneral, zValidator('json', updateSectionSchema, (result, c) => {
    if (!result.success) {
        const msg = result.error.issues[0]?.message || 'Invalid request';
        return c.json({ ok: false, error: msg }, 400);
    }
    ...
    try {
        ...
        function replaceSectionContent(fileContent) {
        ...
        if (isLocal || date) {
            ...
            try {
                ...
                console.error('[memories] Gateway section update failed:', err.message);
        ...
        console.error('[memories] PUT section error:', err.message);
        return c.json({ ok: false, error: 'Failed to update memory section' }, 500);
    }
});
/**
 ...
 */
app.delete('/api/memories', rateLimitGeneral, zValidator('json', deleteMemorySchema, (result, c) => {
    if (!result.success) {
        const msg = result.error.issues[0]?.message || 'Invalid request';
        return c.json({ ok: false, error: msg }, 400);
    }
    ...
    try {
        ...
        if (isLocal || isDailyType) {
            ...
            try {
                ...
                console.error('[memories] Gateway DELETE failed:', err.message);
