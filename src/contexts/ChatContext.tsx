...
export function ChatProvider({ children }: { children: ReactNode }) {
  ...
  // Composed recovery hook methods are intentionally stable; re-running on
  // every hook object identity change would create reconnect churn.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
    connectionState,
    currentSession,
  ...
  // Load history only when the active session or connection state changes.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  // Polling depends on the active subagent lifecycle, not the composed hook object.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  // The watchdog keys off timestamp + generation state; the composed recovery
  // hook object itself is not a meaningful dependency.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
    isGenerating,
  ...
  // This subscription intentionally tracks the stable callback methods it calls.
  // Depending on the composed hook objects would resubscribe on every stream tick.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
