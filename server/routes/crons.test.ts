...
describe('cron routes', () => {
  ...
  it('runs isolated agentTurn jobs through sessions_spawn', async () => {
    ...
    const runLedger = await fs.readFile(join(tempHome, '.robin', 'cron', 'robin-manual-runs', 'job-123.jsonl'), 'utf8');
  ...
  it('merges manual run history entries into cron runs', async () => {
    ...
    await fs.mkdir(join(tempHome, '.robin', 'cron', 'robin-manual-runs'), { recursive: true });
    await fs.writeFile(
      join(tempHome, '.robin', 'cron', 'robin-manual-runs', 'job-123.jsonl'),
