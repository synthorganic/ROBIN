#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────────────
# ROBIN Installer — one-command setup for the ROBIN web interface
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/daggerhashimoto/openclaw-robin/master/install.sh | bash
#
# Or with options:
#   curl -fsSL ... | bash -s -- --dir ~/robin --version v1.4.4
#   curl -fsSL ... | bash -s -- --dir ~/robin --branch main
...
# ── Defaults ──────────────────────────────────────────────────────────
INSTALL_DIR="${ROBIN_INSTALL_DIR:-${HOME}/robin}"
BRANCH="master"
BRANCH_EXPLICIT=false
VERSION=""
REPO="https://github.com/daggerhashimoto/openclaw-robin.git"
NODE_MIN=22
SKIP_SETUP=false
DRY_RUN=false
GATEWAY_TOKEN=""
...
run_with_dots() {
  local msg="$1"; shift
  local stderr_file
  stderr_file=$(mktemp /tmp/robin-rwd-XXXXXX)
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help|-h)
      ...
      echo ""
      echo "Options:"
      echo "  --dir <path>         Install directory (default: ~/robin)"
...
if [[ "$DRY_RUN" == "true" ]]; then
  ...
  if [[ -d "$INSTALL_DIR/.git" ]]; then
    ...
    if [[ "$TARGET_REF_KIND" == "branch" || "$TARGET_REF_KIND" == "branch-fallback" ]]; then
      run_with_dots "Cloning ROBIN" git clone --branch "$TARGET_REF" --depth 1 -q "$REPO" "$INSTALL_DIR"
    else
      run_with_dots "Cloning ROBIN" git clone --depth 1 -q "$REPO" "$INSTALL_DIR"
      cd "$INSTALL_DIR"
      run_with_dots "Fetching tags" git fetch --tags origin -q
      run_with_dots "Checking out ${TARGET_REF}" git checkout --force "$TARGET_REF" -q
    fi
...
if [[ "$DRY_RUN" == "true" ]]; then
  ...
  npm_log=$(mktemp /tmp/robin-npm-install-XXXXXX)
  ...
  if [[ $RWD_EXIT -eq 0 ]]; then
    ...
    if [[ -d dist || -d server-dist ]]; then
      BUILD_BACKUP=$(mktemp -d /tmp/robin-build-backup-XXXXXX)
  ...
  build_log=$(mktemp /tmp/robin-build-XXXXXX)
  ...
  WHISPER_MODEL_DIR="${HOME}/.robin/models"
...
generate_env_from_gateway() {
  ...
  if [[ -n "$gw_token" ]]; then
    local robin_port=3080
    if ! check_port "$robin_port"; then
      if [[ "$INTERACTIVE" != "true" ]]; then
        fail "Port ${robin_port} is already in use. Set a different PORT in .env or free the port."
        exit 1
      else
        warn "Port ${robin_port} is already in use"
        while true; do
          printf "  ${RAIL}  ${CYAN}→${NC} Enter an available port: "
          if ! read -r robin_port < /dev/tty 2>/dev/null; then
            fail "Cannot read from terminal"
            exit 1
          fi
          if [[ ! "$robin_port" =~ ^[0-9]+$ ]] || (( robin_port < 1 || robin_port > 65535 )); then
          ...
          warn "Port ${robin_port} is also in use"
        done
      fi
    fi
    cat > .env <<ENVEOF
    ...
    if [[ -f .env ]]; then
      if [[ "$INTERACTIVE" == "true" && -z "$ACCESS_MODE" ]]; then
        ...
        if read -r answer < /dev/tty 2>/dev/null; then
          if [[ "$(echo "$answer" | tr "[:upper:]" "[:lower:]")" == "y" ]]; then
            echo ""
            ROBIN_INSTALLER=1 npm run setup < /dev/tty 2>/dev/null || {
      ...
      ROBIN_INSTALLER=1 npm run setup -- --defaults --access-mode "$ACCESS_MODE" || {
...
setup_systemd() {
  local service_file="/etc/systemd/system/robin.service"
  local node_bin
  node_bin=$(which node)
  ...
  local tmp_service
  tmp_service=$(mktemp /tmp/robin.service.XXXXXX)

  cat > "$tmp_service" <<EOF
[Unit]
Description=ROBIN - OpenClaw Web UI
...
EOF
  ...
  if [[ $EUID -eq 0 ]]; then
    ...
    if [[ -f "${working_dir}/.env" ]]; then
      run_with_dots "Systemd service" bash -c "systemctl daemon-reload && systemctl enable robin.service &>/dev/null && systemctl start robin.service"
      if [[ $RWD_EXIT -eq 0 ]]; then
        ok "Systemd service installed and started"
      else
        warn "Systemd service install failed — try: sudo systemctl start robin.service"
      fi
    else
      systemctl daemon-reload
      systemctl enable robin.service &>/dev/null
      ok "Systemd service installed (not started — run ${CYAN}npm run setup${NC} first, then ${CYAN}systemctl start robin.service${NC})"
    fi
  else
    echo ""
    ...
    echo "    sudo systemctl enable robin.service"
    echo "    sudo systemctl start robin.service"
    echo ""
...
setup_launchd() {
  ...
  local plist_file="${plist_dir}/com.robin.server.plist"
...
#!/bin/bash
# ROBIN start wrapper — .env is loaded by the Node server at runtime.
...
<dict>
  <key>Label</key>
  <string>com.robin.server</string>
  ...
  <string>${working_dir}/robin.log</string>
...
EOF
  ...
  if launchctl bootstrap "gui/${uid}" "$plist_file" 2>/dev/null; then
    ...
    info "ROBIN will start automatically on login"
  ...
  echo ""
  info "Manage:"
  echo "    launchctl stop com.robin.server"
  echo "    launchctl start com.robin.server"
...
if [[ "$(uname -s)" == "Darwin" ]]; then
  ...
  plist_check="${HOME}/Library/LaunchAgents/com.robin.server.plist"
  if [[ "$DRY_RUN" == "true" ]]; then
    if [[ -f "$plist_check" ]]; then
      dry "launchd service already exists — would restart it"
    else
      dry "Would create launchd service (~/Library/LaunchAgents/com.robin.server.plist)"
    fi
  else
    ...
    if [[ -f "$plist_check" ]]; then
      ...
      uid=$(id -u)
      launchctl bootout "gui/${uid}/com.robin.server" 2>/dev/null || launchctl stop com.robin.server 2>/dev/null || true
      setup_launchd
    elif [[ "$INTERACTIVE" == "true" ]]; then
  ...
  if [[ "$DRY_RUN" == "true" ]]; then
    if [[ -f /etc/systemd/system/robin.service ]]; then
      dry "Service already exists — would restart it"
    else
      dry "Would prompt to install systemd service"
      dry "Would create /etc/systemd/system/robin.service"
    ...
    if [[ -f /etc/systemd/system/robin.service ]]; then
      ...
      if [[ $EUID -eq 0 ]]; then
        systemctl stop robin.service 2>/dev/null || true
      else
        sudo systemctl stop robin.service 2>/dev/null || true
      fi
      setup_systemd
    elif [[ "$INTERACTIVE" == "true" ]]; then
...
if [[ "$DRY_RUN" == "true" ]]; then
  ...
  header_len=29  # "Open ROBIN in your browser:" + padding
  ...
  echo ""
  echo -e "     ${GREEN}${BOLD}✅ ROBIN installed!${NC}"
  echo ""
  echo -e "     ${ORANGE}╭$(printf '─%.0s' $(seq 1 $box_inner))╮${NC}"
  echo -e "     ${ORANGE}│${NC}$(printf ' %.0s' $(seq 1 $box_inner))${ORANGE}│${NC}"
  echo -e "     ${ORANGE}│${NC}  ${BOLD}Open ROBIN in your browser:${NC}$(printf ' %.0s' $(seq 1 $((box_inner - 29))))${ORANGE}│${NC}"
  ...
  if $IS_MAC; then
    echo -e "     ${DIM}Restart:   launchctl stop com.robin.server && launchctl start com.robin.server${NC}"
    echo -e "     ${DIM}Logs:      tail -f ${INSTALL_DIR}/robin.log${NC}"
  elif command -v systemctl &>/dev/null; then
    echo -e "     ${DIM}Restart:   sudo systemctl restart robin.service${NC}"
    echo -e "     ${DIM}Logs:      sudo journalctl -u robin.service -f${NC}"
  else
...
if [[ "$ENV_MISSING" == "true" ]] || [[ ! -f "${INSTALL_DIR}/.env" ]]; then
  warn "Install complete but ROBIN is not fully configured"
