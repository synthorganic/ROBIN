...
function normalizeAlias(entry) {
    const alias = entry?.alias;
    return typeof alias === 'string' && alias.trim() ? alias.trim() : undefined;
}
function buildGatewayModelInfo(id, role, entry) {
...
async function getModelCatalog() {
    ...
    try {
        ...
        console.warn('[gateway/models] failed to read configured models from config:', configPath, err.message);
 ...
 */
function extractThinking(payload) {
    ...
    const runtime = p.runtime || p.agent?.runtime;
 ...
 */
function extractSessionModel(payload) {
    if (!payload || typeof payload !== 'object')
        return null;
    const p = payload;
    const candidates = [
...
function getGatewaySessionKey(session) {
    return session.sessionKey || session.key || '';
}
function isTopLevelAgentSessionKey(sessionKey) {
    return /^agent:[^:]+:main$/.test(sessionKey);
}
function pickPreferredSessionKey(sessions) {
    ...
    if (firstRoot)
        return getGatewaySessionKey(firstRoot);
...
app.get('/api/gateway/session-info', rateLimitGeneral, async (c) => {
    ...
    try {
        const result = await invokeGatewayTool('sessions_list', { activeMinutes: SESSIONS_ACTIVE_MINUTES, limit: SESSIONS_LIMIT }, GATEWAY_TIMEOUT_MS);
        ...
        if (session) {
            ...
            if (thinking)
                info.thinking = thinking.toLowerCase();
        }
        if (info.model || info.thinking)
            return c.json(info);
    }
    catch (err) {
        console.warn(`[gateway/session-info] sessions_list failed:`, err.message);
    }
    ...
    for (const tool of toolsToTry) {
        try {
            ...
            if (thinking && !info.thinking)
                info.thinking = thinking;
            if (model && !info.model)
                info.model = model;
            if (info.thinking && info.model)
                return c.json(info);
        }
        catch (err) {
            console.warn(`[gateway/session-info] ${tool} failed:`, err.message);
        }
    }
    return c.json(info);
});
...
app.post('/api/gateway/session-patch', rateLimitGeneral, async (c) => {
    ...
    if (!sessionKey) {
        try {
            const listResult = await invokeGatewayTool('sessions_list', { activeMinutes: SESSIONS_ACTIVE_MINUTES, limit: SESSIONS_LIMIT }, GATEWAY_TIMEOUT_MS);
            const r = listResult;
            const sessions = (Array.isArray(r.sessions)
                ? r.sessions
                : Array.isArray(r.details?.sessions)
            ...
            console.warn('[gateway/session-patch] sessions_list fallback failed:', err.message);
        }
    }
    if (!sessionKey) {
        return c.json({ ok: false, error: 'No active root session available. Provide sessionKey explicitly.' }, 409);
    }
    // Change model via session_status tool (reliable — uses HTTP tools/invoke)
    if (body.model) {
        try {
            const statusResult = await invokeGatewayTool('session_status', { model: body.model, sessionKey }, GATEWAY_TIMEOUT_MS);
            ...
            console.warn('[gateway/session-patch] session_status model change failed:', err.message);
            result.ok = false;
            result.error = `Model change failed: ${err.message}`;
