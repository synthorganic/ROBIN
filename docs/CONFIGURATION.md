...
Connects ROBIN to your local gateway (ROBIN gateway v1 or compatible). The wizard auto-detects the gateway token from:
1. Existing `.env` (`GATEWAY_TOKEN`)
2. Gateway config file `~/.robin/gateway.json` (auto-detected)
...
> **Note:** ROBIN uses `GATEWAY_TOKEN` only. No OpenClaw gateway fallbacks exist.
>
> **Token detection order:** The setup wizard finds the gateway token from: (1) `~/.robin/gateway.json` (generated by `npm run gateway-init`), (2) `GATEWAY_TOKEN` shell env var.
...
| `MEMORY_PATH` | `~/.robin/workspace/MEMORY.md` | Path to the main agent's long-term memory file |
| `MEMORY_DIR` | `~/.robin/workspace/memory/` | Directory for the main agent's daily memory files (`YYYY-MM-DD.md`) |
| `SESSIONS_DIR` | `~/.robin/agents/main/sessions/` | Session transcript directory (scanned for token usage) |
| `USAGE_FILE` | `~/.robin/token-usage.json` | Persistent cumulative token usage data |
...
MEMORY_PATH=/home/user/.robin/workspace/MEMORY.md
MEMORY_DIR=/home/user/.robin/workspace/memory
SESSIONS_DIR=/home/user/.robin/agents/main/sessions
```
