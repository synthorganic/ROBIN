 ...
 *
 * WebSocket traffic is proxied through ROBIN's `/ws` endpoint so the
...
export function useWebSocket(): UseWebSocketReturn {
  ...
  const doConnect = useCallback((url: string, token: string, isReconnect: boolean): Promise<void> => {
    return new Promise((resolve, reject) => {
      ...
      let ws: WebSocket;
      try {
        // Always proxy WebSocket through ROBIN's /ws endpoint.
        // This ensures the connection works regardless of how the user
        // accesses ROBIN (direct, SSH tunnel, reverse proxy, HTTPS).
