...
export interface EnvConfig {
  ...
  // Authentication
  ROBIN_AUTH?: string;
  ROBIN_PASSWORD_HASH?: string;
  ROBIN_SESSION_SECRET?: string;
  ROBIN_SESSION_TTL?: string;
}
...
export function generateEnvContent(config: EnvConfig): string {
  ...
  lines.push('# Compatible Agent Gateway');
  ...
  // Authentication
  if (config.ROBIN_AUTH === 'true' || config.ROBIN_PASSWORD_HASH || config.ROBIN_SESSION_SECRET) {
    const authLines: string[] = [];
    authLines.push(`ROBIN_AUTH=${config.ROBIN_AUTH || 'false'}`);
    if (config.ROBIN_PASSWORD_HASH) authLines.push(`ROBIN_PASSWORD_HASH=${config.ROBIN_PASSWORD_HASH}`);
    if (config.ROBIN_SESSION_SECRET) authLines.push(`ROBIN_SESSION_SECRET=${config.ROBIN_SESSION_SECRET}`);
    if (config.ROBIN_SESSION_TTL) authLines.push(`ROBIN_SESSION_TTL=${config.ROBIN_SESSION_TTL}`);
