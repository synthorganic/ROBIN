...
export type ParticipantType = 'agent' | 'assistant' | 'user' | 'system' | string;
...
export interface ChatMessage {
  ...
  content: string;
  type?: 'text' | 'tool-call' | 'tool-result' | 'code' | 'system';
...
class SharedChatService {
  ...
  private config: Required<SharedChatConfig>;

  constructor(config?: SharedChatConfig) {
    this.config = {
   ...
   */
  getRecentMessages(options?: { limit?: number }): ChatMessage[] {
   ...
   */
  getParticipants(): Map<string, ParticipantState> {
