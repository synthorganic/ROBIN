...
async function readAgentLog() {
    return readJSON(config.agentLogPath, []);
}
async function writeAgentLog(entries) {
...
app.get('/api/agentlog', async (c) => {
    try {
        ...
        console.error('[agentlog] read error:', err.message);
        return c.json([]);
    }
});
app.post('/api/agentlog', rateLimitGeneral, zValidator('json', agentLogSchema, (result, c) => {
    if (!result.success) {
