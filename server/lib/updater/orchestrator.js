 ...
 */
export async function orchestrate(options, reporter) {
    if (options.rollback) {
    ...
    try {
        ...
        const sourceLabel = resolved.source === 'release'
            ? 'latest release'
            : resolved.source === 'tag'
                ? 'latest tag fallback'
                : 'pinned version';
        ...
        if (!options.yes) {
            ...
            const confirmed = await reporter.confirm(`Update v${resolved.current} → v${resolved.version}?`);
            if (!confirmed) {
        ...
        if (!options.noRestart) {
            ...
            if (serviceManager) {
                ...
                if (!active) {
                    throw new UpdateError(`Service failed to start via ${serviceManager.name}`, 'restart', EXIT_CODES.RESTART);
                }
            ...
            if (serviceManager) {
                ...
                if (!health.healthy || !health.versionMatch) {
                    throw new UpdateError(health.error ?? 'Health check failed', 'health', EXIT_CODES.HEALTH);
                }
...
// ── Failure handler ──────────────────────────────────────────────────
async function handleFailure(err, options, serviceManager, snapshotCreated, reporter) {
    const isUpdateError = err instanceof UpdateError;
    const stage = isUpdateError ? err.stage : 'unknown';
    const message = err instanceof Error ? err.message : String(err);
    let exitCode = isUpdateError ? err.exitCode : EXIT_CODES.BUILD;
...
// ── Manual rollback ──────────────────────────────────────────────────
async function handleManualRollback(options, reporter) {
    let locked = false;
    try {
...
function writeLastRun(data) {
    try {
        ...
        writeFileSync(join(STATE_DIR, 'last-run.json'), JSON.stringify({ timestamp: Date.now(), ...data }, null, 2), 'utf-8');
    }
    catch {
