...
import { listAvailableSemverVersions, normalizeSemverTag, resolveLatestVersion, } from '../release-source.js';
 ...
 */
export async function resolveVersion(cwd, explicitVersion) {
    ...
    if (explicitVersion) {
        ...
        if (!normalized) {
            throw new UpdateError(`Invalid version ${explicitVersion} (expected vX.Y.Z)`, 'resolve', EXIT_CODES.VERSION_RESOLUTION);
        }
        const available = listAvailableSemverVersions(cwd);
        if (!available.includes(normalized)) {
            throw new UpdateError(`Tag v${normalized} not found (available: ${available.map(v => `v${v}`).join(', ') || 'none'})`, 'resolve', EXIT_CODES.VERSION_RESOLUTION);
        }
        version = normalized;
        tag = `v${version}`;
        source = 'explicit';
        ...
        if (!latest) {
            throw new UpdateError('No published release or semver tags found', 'resolve', EXIT_CODES.VERSION_RESOLUTION);
        }
