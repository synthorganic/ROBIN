...
import { getWorkspaceRoot, resolveWorkspacePathForRoot, isExcluded, isBinary, MAX_FILE_SIZE, } from '../lib/file-utils.js';
import { config } from '../lib/config.js';
import { FileOpError, moveEntry, renameEntry, restoreEntry, trashEntry, } from '../lib/file-ops.js';
...
const app = new Hono();
// ── Helpers ──────────────────────────────────────────────────────────
function resolveScopedWorkspace(agentId) {
...
async function listDirectory(dirPath, basePath, depth) {
    const entries = [];
    let items;
    try {
...
function handleFileOpError(c, err) {
...
function gatewayFilesToTree(files) {
    return files
        .filter((f) => !f.missing)
        .map((f) => ({
        name: f.name,
...
app.get('/api/files/tree', async (c) => {
    ...
    try {
        ...
        console.warn('[file-browser] Gateway tree fallback failed:', err.message);
        return c.json({
            ok: true,
            root: '.',
            entries: [],
...
app.get('/api/files/resolve', async (c) => {
    ...
    const resolved = await resolveWorkspacePathForRoot(workspace.workspaceRoot, targetPath, { allowNonExistent: true });
    if (!resolved) {
...
app.put('/api/files/write', async (c) => {
    ...
    try {
        ...
        console.error('[file-browser] Gateway write fallback failed:', err.message);
        return c.json({ ok: false, error: 'Failed to write file' }, 500);
    }
});
// ── Mutation endpoints — 501 for remote workspaces ───────────────────
async function requireLocalWorkspace(c, workspace) {
    if (!(await isWorkspaceLocal(workspace.workspaceRoot))) {
        return c.json({
            ok: false,
            error: 'Not supported for remote workspaces',
