...
import { opsTerminalManager, type OpsTerminalId } from '../lib/ops-terminals.js';

const app = new Hono();

function parseTerminalId(id: string): OpsTerminalId {
  if (id === 'cli' || id === 'support' || id === 'logs') return id;
...
app.post('/api/terminals/:id/resize', async (c) => {
  ...
  try {
    return c.json({
      ok: true,
      terminal: opsTerminalManager.resize(
        parseTerminalId(c.req.param('id')),
        Number(body.cols) || 120,
        Number(body.rows) || 28,
      ),
    });
  } catch (error) {
