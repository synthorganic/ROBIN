...
function buildQwenInput(text, voice) {
    ...
    if (qwen.mode === 'voice_design') {
        if (qwen.voiceDescription)
            input.voice_description = qwen.voiceDescription;
        if (qwen.styleInstruction)
            input.style_instruction = qwen.styleInstruction;
    }
 ...
 */
export async function synthesizeReplicate(text, options) {
