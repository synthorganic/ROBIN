...
let gatewayStartedAtCache = null;
let cacheTs = 0;
 ...
 */
async function getGatewayStartedAt() {
    if (!isLinux)
        return null; // /proc and pgrep are Linux-only
    const now = Date.now();
    if (gatewayStartedAtCache && now - cacheTs < CACHE_TTL)
