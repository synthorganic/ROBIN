...
async function downloadModel(model) {
    ...
    const progress = { model, downloading: true, bytesDownloaded: 0, totalBytes: 0, percent: 0 };
    ...
    try {
        ...
        const msg = isAbort ? 'Download cancelled' : err.message;
        if (!isAbort) {
 ...
 */
export async function transcribeLocal(fileData, filename, language) {
    // Pre-flight checks
    if (!isModelAvailable()) {
        return {
            ok: false,
    ...
    if (effectiveLang !== 'en' && isEnModel) {
        console.warn(`[whisper-local] Language "${effectiveLang}" requested but model "${activeModel}" is English-only. ` +
    ...
    try {
        ...
        try {
            ...
            console.error('[whisper-local] ffmpeg conversion failed:', err.message);
        ...
        console.error('[whisper-local] Transcription failed:', err.message);
        return { ok: false, status: 500, message: 'Local transcription failed: ' + err.message };
    }
    finally {
