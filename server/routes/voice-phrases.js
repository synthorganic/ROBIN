...
import { getVoicePhrases, getLanguagePhrases, hasCustomPhrases, setLanguagePhrases, } from '../lib/voice-phrases.js';
const app = new Hono();
/**
...
app.get('/api/voice-phrases/:lang', (c) => {
    ...
    if (custom)
        return c.json({ source: 'custom', ...custom });
    const defaults = DEFAULT_VOICE_PHRASES[lang];
    if (defaults)
        return c.json({ source: 'defaults', ...defaults });
...
app.put('/api/voice-phrases/:lang', async (c) => {
    ...
    if (!valid)
        return c.text(`Unsupported language: ${lang}`, 400);
    try {
