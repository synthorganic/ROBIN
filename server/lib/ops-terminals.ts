...
export type OpsTerminalId = 'cli' | 'support' | 'logs';
export type OpsWorkspaceRestartMode = 'none' | 'running';

export interface OpsTerminalState {
  id: OpsTerminalId;
  label: string;
...
export interface OpsWorkspaceSwitchResult {
  path: string;
  previousPath: string;
  restartedTerminals: Array<Exclude<OpsTerminalId, 'logs'>>;
  terminals: OpsTerminalState[];
}
...
function resolveCliAgentRoot(baseDir: string) {
  const candidates = [
    path.resolve(baseDir, 'vendor', 'cli-agent'),
  ];

  return candidates.find((candidate) => (
    fs.existsSync(path.join(candidate, 'cli.js'))
    && fs.existsSync(path.join(candidate, 'package.json'))
  )) ?? null;
}

function resolveCliLaunchScript(baseDir: string) {
...
export class OpsTerminalManager {
  private readonly states = new Map<OpsTerminalId, OpsTerminalState>();
  private readonly ptys = new Map<Exclude<OpsTerminalId, 'logs'>, IPty>();
  private workspacePath = defaultTerminalWorkspace();
  private readonly cliAgentRoot = resolveCliAgentRoot(path.resolve(process.cwd()));
  private readonly cliLaunchScript = resolveCliLaunchScript(path.resolve(process.cwd()));
  private readonly codexExecutable = resolveExecutable(
  ...
  setWorkspace(nextPath: string, options?: { restart?: OpsWorkspaceRestartMode }): OpsWorkspaceSwitchResult {
    if (!nextPath?.trim()) {
  ...
  start(id: OpsTerminalId): OpsTerminalState {
    ...
    pty.onExit(({ exitCode }) => {
      if (this.ptys.get(id) !== currentPty) return;
      this.ptys.delete(id);
      state.running = false;
      state.pid = null;
      state.lastUpdated = now();
  ...
  stop(id: OpsTerminalId): OpsTerminalState {
    const state = this.require(id);
  ...
  write(id: OpsTerminalId, input: string): OpsTerminalState {
    const state = this.require(id);
    if (state.readonly) {
      throw new Error(`${state.label} is read-only`);
    }

    const pty = this.ptys.get(id as Exclude<OpsTerminalId, 'logs'>);
    if (!pty) {
  ...
  sendBlock(id: Exclude<OpsTerminalId, 'logs'>, text: string): OpsTerminalState {
    const payload = text.replace(/\r?\n/g, '\r') + '\r';
    return this.write(id, payload);
  }

  resize(id: OpsTerminalId, cols: number, rows: number): OpsTerminalState {
    const state = this.require(id);
  ...
  ctrlC(id: Exclude<OpsTerminalId, 'logs'>): OpsTerminalState {
    return this.write(id, '\x03');
  }

  appendLog(message: string): OpsTerminalState {
  ...
  private launchSpec(id: Exclude<OpsTerminalId, 'logs'>): TerminalLaunchSpec {
  ...
  private require(id: OpsTerminalId) {
    const state = this.states.get(id);
    if (!state) throw new Error(`Unknown terminal '${id}'`);
    return state;
  }

  private emitStatus(state: OpsTerminalState) {
  ...
  private pushOutput(id: OpsTerminalId, chunk: string) {
    const state = this.require(id);
  ...
  private clone(state: OpsTerminalState): OpsTerminalState {
    return {
      ...state,
      buffer: [...state.buffer],
    };
...
opsTerminalManager.appendLog(
  `[ROBIN] terminal bus online on ${os.hostname()} (${config.host}:${config.port})`,
);
