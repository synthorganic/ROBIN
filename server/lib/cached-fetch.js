 ...
 */
export function createCachedFetch(fetcher, ttlMs = DEFAULT_TTL_MS, opts) {
    const slot = { data: null, ts: 0, ttl: ttlMs, inFlight: null };
    const FAILURE_TTL_MS = 30_000; // retry failures after 30s, not 5min
    return async () => {
        const now = Date.now();
        if (slot.data !== null && now - slot.ts < slot.ttl)
            return slot.data;
        if (!slot.inFlight) {
            slot.inFlight = fetcher().then((result) => {
