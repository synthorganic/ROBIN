...
export function normalizeSemverTag(tag) {
    if (!tag)
        return null;
    const match = SEMVER_TAG_REGEX.exec(tag.trim());
    return match ? match[1] : null;
}
function parseSemverTags(output) {
    const versions = [];
...
function fetchSemverTagsFromCommand(cwd, command) {
    try {
...
export async function latestReleaseVersion(cwd) {
    ...
    try {
        const payload = await requestJson(`https://api.github.com/repos/${repo.owner}/${repo.repo}/releases/latest`, headers, RELEASE_REQUEST_TIMEOUT_MS);
        const tagName = payload && typeof payload === 'object' && 'tag_name' in payload && typeof payload.tag_name === 'string'
            ? payload.tag_name
            : null;
...
export async function resolveLatestVersion(cwd) {
