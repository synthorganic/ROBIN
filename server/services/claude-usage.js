 ...
 */
async function pollFor(getBuffer, test, timeoutMs, intervalMs = 500) {
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
        if (test(stripAnsi(getBuffer())))
            return true;
...
function hasTrustPrompt(clean) {
    const lower = clean.toLowerCase();
    return (lower.includes('trust this folder') ||
...
export async function getRobinOpsUsage() {
    ...
    try {
        ...
        const mPanelSess = output.match(/Current\s*session[\s\S]*?(\d+)%\s*used[\s\S]*?Rese?t?s?\s*([^\n]+)/i);
        if (mPanelSess) {
            sessionUsed = parseInt(mPanelSess[1], 10);
            sessionResets = mPanelSess[2].trim();
        }
        // Weekly — same flexible matching across line boundaries
        const mPanelWeek = output.match(/Current\s*week(?:\s*\([^)]*\))?[\s\S]*?(\d+)%\s*used[\s\S]*?Rese?t?s?\s*([^\n]+)/i);
        if (mPanelWeek) {
        ...
        return {
            ...
            session_limit: sessionUsed !== null
                ? {
