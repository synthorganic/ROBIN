 ...
 */
function createGatewayRelay(clientWs, targetUrl, clientOrigin, connId, isTrusted) {
    const tag = `[ws-proxy:${connId}]`;
    ...
    function enqueuePending(data, isBinary) {
    ...
    function updateClientKindFromConnect(msg) {
     ...
     */
    function dispatchConnect(nonce) {
        ...
        if (isTrusted && config.gatewayToken && !modified.params?.auth?.token) {
            modified = {
                ...modified,
                params: {
                    ...modified.params,
                    auth: {
                        ...modified.params?.auth,
    ...
    function startChallengeDeadline() {
        clearChallengeTimer();
    ...
    function openGateway() {
        ...
        gwWs.on('error', (err) => {
            ...
            clearChallengeTimer();
            if (!hasRetried || handshakeComplete)
                clientWs.close();
        });
        gwWs.on('close', (code, reason) => {
            ...
            const isDeviceRejection = code === 1008 && (reasonStr.includes('device token mismatch') ||
...
function injectDeviceIdentity(msg, nonce, logTag = '[ws-proxy]') {
    const params = (msg.params || {});
