...
import sharedChatRoutes from './routes/shared-chat.js';
...
import opsAgentRoutes from './routes/ops-agent.js';
import opsTerminalRoutes from './routes/ops-terminals.js';
...
app.use('*', async (c, next) => {
  const isSseRoute = c.req.path === '/api/events'
    || /^\/api\/agent\/session\/.+\/stream$/.test(c.req.path);
  if (isSseRoute) return next();
