...
import { getKanbanStore, VersionConflictError, TaskNotFoundError, InvalidTaskStatusError, InvalidBoardConfigError, InvalidTransitionError, ProposalNotFoundError, ProposalAlreadyResolvedError, } from '../lib/kanban-store.js';
...
import { buildKanbanFallbackRunKey, launchKanbanFallbackSubagentViaRpc, } from '../lib/kanban-subagent-fallback.js';
const app = new Hono();
...
function parseGatewayResponse(result) {
...
export async function cleanupKanbanPollers() {
    ...
    for (const t of activePollTimers)
        clearTimeout(t);
 ...
 */
function shouldUseKanbanFallback() {
...
function findGatewayRunMatch(sessions, identity) {
    if (identity.childSessionKey) {
        const byChildSessionKey = sessions.find((session) => (String(session.childSessionKey ?? session.sessionKey ?? session.sessionId ?? '') === identity.childSessionKey));
        if (byChildSessionKey)
            return byChildSessionKey;
    }
    if (identity.runId) {
...
function isFallbackChildSession(sessionKey, parentSessionKey) {
    const parentMatch = parentSessionKey.match(/^agent:([^:]+):main$/);
    if (!parentMatch)
        return false;
    return sessionKey.startsWith(`agent:${parentMatch[1]}:subagent:`);
}
function pickSpawnedChildSession(sessions, identity) {
    const unseenChildren = sessions.filter((session) => {
        const sessionKey = getSessionKey(session);
        if (!sessionKey)
            return false;
        if (!isFallbackChildSession(sessionKey, identity.parentSessionKey))
            return false;
...
function getLastAssistantText(messages, fallback = 'Completed (no result text)') {
...
function trimKanbanParentReportText(text, maxChars = 4_000) {
...
function pollSessionCompletion(store, taskId, identity, intervalMs = 5_000, maxAttempts = 720) {
    let attempts = 0;
    const poll = async () => {
        attempts++;
        ...
        try {
            ...
            if (status === 'done') {
                ...
                if (!childSessionKey) {
                    ...
                    try {
                        ...
                        const messages = (histParsed.messages ?? []);
            ...
            if (status === 'error' || status === 'failed') {
                const errorMsg = match.error || 'Agent session failed';
...
function pollFallbackSessionCompletion(store, taskId, identity, intervalMs = 5_000, maxAttempts = 720) {
    let attempts = 0;
    const poll = async () => {
        attempts++;
        if (attempts > maxAttempts) {
...
app.patch('/api/kanban/tasks/:id', rateLimitGeneral, async (c) => {
    ...
    const cleanPatch = Object.fromEntries(Object.entries(rawPatch)
...
app.post('/api/kanban/tasks/:id/reorder', rateLimitGeneral, async (c) => {
    ...
    try {
        const task = await store.reorderTask(id, parsed.data.version, parsed.data.targetStatus, parsed.data.targetIndex, 'operator');
        return c.json(task);
    }
    catch (err) {
        const invalidStatusResponse = handleInvalidTaskStatusError(c, err);
...
app.post('/api/kanban/proposals', rateLimitGeneral, async (c) => {
    ...
    if (type === 'create') {
        ...
        try {
            await store.getTask(safePayload.id);
        }
        catch (err) {
            if (err instanceof TaskNotFoundError) {
...
// ── Workflow helpers ──────────────────────────────────────────────────
function handleInvalidTaskStatusError(c, err) {
...
function handleWorkflowError(c, err) {
    const invalidStatusResponse = handleInvalidTaskStatusError(c, err);
    if (invalidStatusResponse)
        return invalidStatusResponse;
...
app.post('/api/kanban/tasks/:id/execute', rateLimitGeneral, async (c) => {
    ...
    try {
        ...
        const execution = await withMutex(`kanban-execute:${id}`, async () => {
...
Deliver your result as a clear summary of what was done.`;
                const task = await store.executeTask(id, {
                    sessionKey,
...
Deliver your result as a clear summary of what was done.`,
        ...
        if (execution.primarySpawn) {
            ...
            if (thinking)
                spawnArgs.thinking = thinking;
            void trackBackgroundTask(invokeGatewayTool('sessions_spawn', spawnArgs)
                .then(async (spawnRaw) => {
        ...
        void trackBackgroundTask((async () => {
            ...
            try {
                ...
                if (launchResult.sessionKey !== fallbackLaunch.sessionKey) {
                    throw new Error(`Run correlation key mismatch, expected ${fallbackLaunch.sessionKey}, got ${launchResult.sessionKey}`);
                }
            }
            catch (err) {
                const errorMessage = err instanceof Error ? err.message : String(err);
                console.error(`[kanban] Failed to launch assignee subagent for task ${id}:`, err);
                await store.completeRun(id, fallbackLaunch.sessionKey, undefined, `Spawn failed: ${errorMessage}`).catch((completeErr) => {
