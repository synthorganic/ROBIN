...
const EMPTY_COST_DATA = { totalCost: 0, totalInput: 0, totalOutput: 0, totalMessages: 0, entries: [] };
const COST_CACHE_TTL = 60_000;
let costCache = { data: EMPTY_COST_DATA, ts: 0 };
 ...
 */
async function scanSessionCosts() {
    const now = Date.now();
    if (costCache.ts && now - costCache.ts < COST_CACHE_TTL)
            ...
            try {
                ...
                for await (const line of rl) {
                    try {
                        ...
                        if (!msg?.usage || !msg.provider || msg.provider === 'openclaw')
                            continue;
