...
export function resolveOpenclawBin() {
    ...
    if (process.env.OPENCLAW_BIN) {
        cached = process.env.OPENCLAW_BIN;
        return cached;
    }
    ...
    console.warn('[openclaw-bin] Could not find openclaw binary. Checked:', candidates.join(', '), '— Set OPENCLAW_BIN env var to fix. Falling back to bare "openclaw" (requires PATH).');
    cached = 'openclaw';
    return cached;
}
