...
function audioResponse(buf, contentType = 'audio/mpeg') {
    return new Response(buf, {
        status: 200,
        headers: { 'Content-Type': contentType },
    });
}
app.post('/api/tts', rateLimitTTS, zValidator('json', ttsSchema, (result, c) => {
    if (!result.success) {
    ...
    try {
        ...
        const ct = 'contentType' in result ? result.contentType : 'audio/mpeg';
...
function validateTTSPatch(patch) {
    if (typeof patch !== 'object' || patch === null || Array.isArray(patch)) {
        return 'Body must be a JSON object';
    }
    for (const [key, val] of Object.entries(patch)) {
        if (!(key in TTS_CONFIG_SCHEMA))
        ...
        for (const [subKey, subVal] of Object.entries(val)) {
            if (!allowed.includes(subKey))
                return `Unknown key: "${key}.${subKey}"`;
            if (typeof subVal !== 'string')
                return `"${key}.${subKey}" must be a string`;
            if (subVal.length > 2000)
                return `"${key}.${subKey}" exceeds max length (2000)`;
        }
    }
    return null;
}
...
app.put('/api/tts/config', rateLimitGeneral, async (c) => {
    try {
        ...
        console.error('[tts-config] update error:', err.message);
