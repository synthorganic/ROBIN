...
import { DEFAULT_VOICE_PHRASES } from './constants.js';
...
function parseStore(raw) {
    ...
    for (const [lang, val] of Object.entries(raw)) {
        ...
        const v = val;
        const entry = {
            stopPhrases: Array.isArray(v.stopPhrases) ? v.stopPhrases : [],
            cancelPhrases: Array.isArray(v.cancelPhrases) ? v.cancelPhrases : [],
        };
        if (Array.isArray(v.wakePhrases) && v.wakePhrases.length > 0) {
            const primaryWake = v.wakePhrases
...
function readStore() {
    try {
        ...
        if (cached && readPath === cachedPath && stat.mtimeMs === cachedMtime)
            return cached;
 ...
 */
export function getLanguagePhrases(lang) {
    const store = readStore();
 ...
 */
export function hasCustomPhrases(lang) {
    const store = readStore();
 ...
 */
export function setLanguagePhrases(lang, phrases) {
    const store = readStore();
    const entry = {
